...
class TeamModel(Base):
    ...
    active: bool = True
    mmr: int = 0
    # members: Optional[Union[list[PlayerModel], list[ObjectIdField]]] = Field(default=None, exclude=True) # dont need this because it will be a property
    ...
    @property
    def members(self):
        approved_members = old_models.PlayerTeamLinkModel.get_all_by_query({'team': self.id, 'approved': True})
        return [member.player for member in approved_members]

    @classmethod
    def db(cls):
        """ Returns a connection to a collection for this model """
        return TeamsRepo(Database().db)
    ...
    class TeamCarousel(Carousel):
        ...
        def is_mine(inter: discord.Interaction, team: 'TeamModel') -> bool:
            ...
            if inter.user == team.captain:
                return True
            if team.co_captain:
                if inter.user == team.co_captain:
                    return True
            return False
    ...
    class PlayerView(discord.ui.View):
        ...
        async def finish_off_view(self, inter: discord.Interaction): #  TODO: Why is did needed?
            await self.wait()
            self.clear_items()
        ...
        async def leave_team(self, inter: discord.Interaction, button: discord.ui.Button):
            ...
            msg = await inter.channel.send(content="Are you Sure?", view=confirmation)  # TODO: this can go inside confirmation?
            ...
            me = old_models.PlayerTeamLinkModel.get_approved(inter.user)
            me.delete()

            if self.team.co_captain == me:
                self.team.co_captain = None
                self.team.save()
            self.stop()
    ...
    class CoCaptainView(PlayerView):
        ...
        @discord.ui.button(custom_id='approve_joins', label='Approve Joins', style=discord.ButtonStyle.green, row=2)
        async def approve_joins(self, inter: discord.Interaction, button: discord.ui.Button):
            if not (approvals := old_models.PlayerTeamLinkModel.get_one_by_query({'team':self.team.id, 'approved':None})):
                raise ValueError('There are no Approvals Pending')
    ...
    class CaptainView(CoCaptainView):
        ...
        async def update_co_captain(self, inter: discord.Interaction, button: discord.Button):
            ...
            if self.team.captain.discord_id != inter.user.id:
                inter.response.send_message(content='You are not Captain of this team :stop_sign:', ephemeral=True) # TODO: the view already checks for captain, dont need to have this
                self.stop()

            team_captains = [self.team.captain.id, self.team.co_captain.id if self.team.co_captain else None] # TODO: this could be a property
...
class TeamRegisterModal(UpdateGenericModal, title='Register a Team'):
    ...
    async def on_submit(self, inter: discord.Interaction) -> None:  # TODO: or instead, return everything back to the vie
        ...
        if TeamModel.get_one_by_query({'name': self.name.value}):
        ...
        c = old_models.PlayerTeamLinkModel(player=PlayerModel.get_by_discord(inter.user), team=item, approved=True)
        c.save()
...
class TeamRegisterPersistent(discord.ui.View):
    ...
    async def register(self, inter: discord.Interaction, button: discord.ui.Button):
        ...
        if old_models.PlayerTeamLinkModel.get_approved(inter.user):
